...
saved_model_name_cnn = 'cnn_domain_adapt'
...
def train_cnn_question_similarity(cnn, batch_ids, batch_data, word2vec, id2Data_title, id2Data_body, word_to_id_vocab, truncation_val_title, truncation_val_body):
    ...
    candidates_qs_tuples_matrix = construct_qs_matrix_training(sequence_ids, cnn, h0, c0, word2vec, id2Data_title, id2Data_body,
        dict_sequence_lengths, num_differing_questions, word_to_id_vocab, truncation_val_title, truncation_val_body, candidates=True)
    main_qs_tuples_matrix = construct_qs_matrix_training(batch_ids, cnn, h0, c0, word2vec, id2Data_title, id2Data_body,
...
def train_nn_domain_classification(neural_net, cnn, ids_ubuntu, ids_android, word2vec,
    ...
    qs_matrix_ubuntu = construct_qs_matrix_domain_classification(ids_ubuntu, cnn, word2vec,
        ubuntu_id_to_data_title, ubuntu_id_to_data_body, word_to_id_vocab, truncation_val_title, truncation_val_body)
    qs_matrix_android = construct_qs_matrix_domain_classification(ids_android, cnn, word2vec,
...
for epoch in range(num_epochs):
    ...
    for batch in range(1, num_batches + 1):
        ...
        loss_batch_domain_classification = train_nn_domain_classification(neural_net, cnn,
    ...
    torch.save(cnn, '../Pickle/' + saved_model_name_cnn + '_e' + str(epoch) + '_b' + str(batch) + '.pth')